    ...
    this.api.get<AttendanceSummary>(`attendance/monthly/${empId}`, { month, year }).subscribe({
      next: (res: any) => {
        ...
        if (data) {
          ...
          const todayRecord = (data.records || []).find((r: AttendanceRecord) => format(new Date(r.date), 'yyyy-MM-dd') === today);
          if (todayRecord && !todayRecord.checkOutTime) {
            this.isCheckedIn.set(true);
            this.lastActionTime.set(new Date(todayRecord.checkInTime));
          }
        }
      }
    });
